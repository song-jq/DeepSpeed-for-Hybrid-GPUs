...
def replace_transformer_layer(orig_layer_impl, model, checkpoint_dict, config, model_config):
    ...
    def replace_wo_policy(module, all_reduce_linears, prefix="", state_dict=None):
        ...
        def addAllreduceTime(t:int):
            global allreduce_time
            allreduce_time.append(t)
            # if rank == 0 or rank == 2:
            print(f"rank:{rank}, allreduce_time: {sum(allreduce_time[3:])/30} ms")
        ...
        def _replace(child, name, conv_linear_layer):
            ...
            if name in all_reduce_linears:  #both
                ...
                if rank <= 1:
                    new_weight = torch.empty((
                        weight_shape[1] if conv_linear_layer else weight_shape[0],
                        (weight_shape[0] if conv_linear_layer else weight_shape[1]) // mp_size * big_slice // origin_slice,
                    ),
                                             device=child.weight.device,
                                             dtype=child.weight.dtype)
                else:
                ...
                return LinearAllreduce(addAllreduceTime, data, child.bias if child.bias is None else \
